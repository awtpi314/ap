...
public class NumberArray {
    ...
    /**
     * The insertionSort method sorts the parameter <i>inArray</i> using the
     * insertion sort. This sort is one of the easiest to implement, although it is
     * potentially slow.
     * 
    ...
    public void mergeSort(int[] inArray, int left, int right) {
        if (left < right) {
            ...
            mergeSort(inArray, left, middle);
            mergeSort(inArray, middle + 1, right);

            merge(inArray, left, middle, right);
        }
    }
    ...
    private void merge(int[] inArray, int left, int middle, int right) {
        ...
        for (int i = 0; i < sizeRight; i++) {
            rightArray[i] = inArray[middle + i + 1];
        }
